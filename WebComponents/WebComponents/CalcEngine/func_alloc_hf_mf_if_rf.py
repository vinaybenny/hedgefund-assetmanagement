...
Function:  func_alloc_hf_mf_if_rf()

Description: This function defines the allocation of wealth between hedge fund, mutual fund,
...
def func_alloc_hf_mf_if_rf( coeffs, simparamObj, sign = -1.0  ):
        ...
        hedge_coeff = coeffs[2]         # Amount of initial wealth invested in hedge fund        
        riskfree_coeff = simparamObj.initWealth - index_coeff - mutual_coeff - hedge_coeff #Remaining wealth is in the risk-free holdings
       
        # Define the wealth value using the input coefficients
        wealth_val = numpy.multiply(index_coeff,simparamObj.indexfund_val) + numpy.multiply(hedge_coeff,simparamObj.hedgefund_val) + \
        numpy.multiply(mutual_coeff,simparamObj.mutualfund_val) + numpy.multiply(riskfree_coeff,simparamObj.riskfree_val)
